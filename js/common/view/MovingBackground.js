  ...
  return inherit( Node, MovingBackground, {
    ...
    createCloudNode: function( initialCenterX, initialTopY, initialScale ) {
      ...
      this.fallingObjectsModel.selectedFallingObject.positionProperty.lazyLink( function( newPosition, oldPosition ) {
        ...
        // Only move the clouds if we falling object is in its static fall position
        if ( self.fallingObjectsModel.fallingObjectNodeStaticPositionProperty.get() ) {
          ...
          } else if ( cloudNode.top > self.sky.bottom ) {  // If we hit the bottom, then move back up to the top
            cloudNode.setBottom( self.sky.top );
          }
        }
